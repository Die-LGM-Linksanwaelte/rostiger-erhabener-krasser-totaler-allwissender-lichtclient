use crate::networking::connection_engine::{
    ClientSession, ConnectionID, NEXT_CONNECTION_ID, SERVER_STATE, SessionID,
};
    ...
    for stream in listener.incoming() {
        match stream {
            Ok(stream) => {
                ...
                println!(
                    "\x1b[34m[Conn {}] New client connected: {}\x1b[0m",
                    connection_id,
    ...
    F: Fn(String) -> Result<String, String> + Send + Sync + 'static + Clone,
{
    match check_version_compatibility(&mut stream) {
        Err(HandshakeError::InvalidData(e)) => {
            println!(
                "\x1b[91m[Conn {}] Handshake Error: {}. Connection Closed.\x1b[0m",
                connection_id, e
            );
            return;
        }
        ...
        Err(HandshakeError::VersionMismatch {
            ...
            println!(
                "\x1b[91m[Conn {}] Version Mismatch: Client: {}, Server: {}. Connection Closed\x1b[0m",
        ...
        Ok(()) => {
            println!(
                "\x1b[34m[Conn {}] Versions match. Handshake completed.\x1b[0m",
                connection_id
            );
        }
    }
    ...
    let bytes = match stream.read(&mut buffer) {
        Ok(0) => {
            return Err(HandshakeError::InvalidData(
                "Connection was terminated instantly".into(),
            ));
        }
        Ok(b) => b,
    ...
    match bincode::deserialize::<HandshakeRequest>(&buffer[..bytes]) {
        Ok(request) => {
            if request.magic_string != "REKTAL" {
                return Err(HandshakeError::InvalidData(
                    "Wrong magic string. Client is not an Rektal-Client".into(),
                ));
            }
        ...
        Err(_) => Err(HandshakeError::InvalidData(
            "Couldnt deserialize handshake".into(),
        )),
    }
}
    ...
    F: Fn(String) -> Result<String, String> + Send + Sync + 'static + Clone,
{
    ...
    loop {
        let bytes = match stream.read(&mut buffer) {
            Ok(0) => {
                println!(
                    "\x1b[34m[Conn {}] Client disconnected successfully\x1b[0m",
                    connection_id
                );
                break;
            }
            Err(e) => {
                println!(
                    "\x1b[91m[Conn {}] Connection error : {}\x1b[0m",
                    connection_id, e
                );
                break;
            }
        ...
        println!(
            "\x1b[34m[Conn {}] Received Enum: {:?}\x1b[0m",
            connection_id, msg
        );
        ...
        let Some(_) = token else {
            if !matches!(msg, TcpClientMessage::Logout) {
                println!(
                    "[Conn {}] Message discarded: Client is not authorized.",
                    connection_id
                );
            ...
            _ => {
                if let Some(response) = handle_messages(msg, command_handler.clone(), connection_id)
                {
                    if let Err(e) = tx_channel.send(response) {
                        println!(
                            "[Conn {}] Got error while sending response to channel: {}",
                            connection_id, e
                        );
                    }
                }
        ...
        if let Some(session) = state.get_mut(&token) {
            match session.active_connection.as_ref().map(|tuple| tuple.1) {
                Some(active_id) if active_id == connection_id => {
                    ...
                    println!(
                        "[Conn {}] Cleanup complete. Session {} is now sleeping",
                        connection_id, token
                    );
                }
                ...
                Some(active_id) => {
                    println!(
                        "[Conn {}] Cleanup for Session {} aborted. Session already has new Connection {}",
                ...
                None => {
                    println!(
                        "\x1b[31m[Conn {}] Cleanup for Session {} aborted. Session is already sleeping.\x1b[0m",
                        connection_id, token
                    );
                }
            }
    ...
    println!(
        "The Read-Thread  {} is dead! Long live the Read-Thread!",
        connection_id
    );
}
...
fn write_thread(
    ...
    while let Ok(message) = rx_channel.recv() {
        match bincode::serialize(&message) {
            Ok(serialized_response) => {
                if let Err(e) = write_stream.write_all(&serialized_response) {
                    println!(
                        "\x1b[91m[Conn {}] Got error while sending to Client: {} Stopped write-Thread\x1b[0m",
                        connection_id, e
                    );
                    break;
                }
            ...
            Err(e) => {
                println!(
                    "\x1b[91m[Conn {}] Got error while serializing response: {}\x1b[0m",
                    connection_id, e
                );
                break;
            }
        ...
        if let TcpServerMessage::Kicked { reason } = message {
            println!(
                "[Conn {}] Write thread terminating because client was kicked: {}",
    ...
    println!(
        "The Write-Thread {} is dead! Long live the Write-Thread!",
        connection_id
    );
}
...
fn update_login_status(
    ...
    let (new_token, response) = match message {
        TcpClientMessage::Login {
            password,
            user_name,
            user_role,
        } => {
            if password == "" {
                //TODO Passwort irgendwo speichern und hier auslesen
                let mut state = SERVER_STATE.write().unwrap();
                ...
                println!(
                    "\x1b[34m[Conn {}] Logged in with Session-Token {}\x1b[0m",
                    connection_id, new_token
                );

                (
                    Some(new_token),
                    Some(TcpServerMessage::LoginOk { token: new_token }),
                )
            } else {
                println!(
                    "\x1b[33m[Conn {}] User {} wanted to login with wrong password\x1b[0m",
                ...
                (
                    None,
                    Some(TcpServerMessage::LoginFailed {
        ...
        TcpClientMessage::Relogin {
            ...
            if let Some(old_token) = old_token {
                println!(
                    "\x1b[33m[Conn {}] User with ID {} wanted to relog with ID {}, but he was logged \
                in. Relogin ignored\x1b[0m",
                    connection_id, old_token, user_id
                );
                (
                    Some(old_token),
                    Some(TcpServerMessage::ReloginFailed {
                        reason: "Already logged in".into(),
                    }),
                )
            } else if let Some(session) = SERVER_STATE.write().unwrap().get_mut(&user_id) {
                if let Some((old_user_channel, old_connection_id)) =
                    session.active_connection.take()
                {
                    let _ = old_user_channel.send(TcpServerMessage::Kicked {
                        reason: "Newer Connection relogged in with same token".into(),
                    });
                    println!(
                        "\x1b[33m[Conn {}] User was kicked due to newer Connection with same token {}. \
                    Killed old Connection {}\x1b[0m",
                        connection_id, user_id, old_connection_id
                    );
                }
                ...
                println!(
                    "\x1b[34m[Conn {}] User with ID {} relogged in  successfully\x1b[0m",
                    connection_id, user_id
                );
                (
                    Some(*user_id),
                    Some(TcpServerMessage::ReloginOk { token: *user_id }),
                )
            } else {
                println!(
                    "\x1b[33m[Conn {}] User wanted to relogin with ID {}, wich doesnt exist\x1b[0m",
                    connection_id, user_id
                );
                (
                    None,
                    Some(TcpServerMessage::ReloginFailed {
                        reason: "User doesnt exist".into(),
                    }),
                )
            }
        }
        ...
        TcpClientMessage::Logout => {
            if let Some(old_token) = old_token {
                ...
                println!(
                    "\x1b[34m[Conn {}] Client logged out successfully\x1b[0m",
                    connection_id
                );
            } else {
    ...
    if let Some(response) = response {
        if let Err(e) = tx_channel.send(response) {
            println!(
                "[Conn {}] Fehler beim Senden der Auth-Antwort an den Channel: {}",
                connection_id, e
            );
        }
    }
...
fn handle_messages<F>(
    msg: TcpClientMessage,
    command_handler: F,
    connection_id: ConnectionID,
) -> Option<TcpServerMessage>
where
    F: Fn(String) -> Result<String, String> + Send + Sync + 'static + Clone,
{
    match msg {
        TcpClientMessage::Login { .. }
        | TcpClientMessage::Logout
        | TcpClientMessage::Relogin { .. } => unreachable!(),
        ...
        TcpClientMessage::Unsubscribe { topic } => {
            println!(
                "\x1b[34m[Conn {}] Der Client will nichts von {} wissen.\x1b[0m",
                connection_id,
                match topic {
                    SubscribeTopic::FixturePositions => "Fixture Positions",
                    SubscribeTopic::Universes => "Universes",
        ...
        TcpClientMessage::ExecuteCommand {
            command,
            terminal_id,
        } => {
        ...
        TcpClientMessage::RequestEdit(_) => {
            println!(
                "\x1b[34m[Conn {}] Yo, Client wollte resource, yo\x1b[0m",
                connection_id
            );
            None
        }
        ...
        TcpClientMessage::SubmitEdit { resource, new_data } => {
            println!(
                "\x1b[34m[Conn {}] Yo, der Nutzer ist fertig mit Resource, yo\x1b[0m",
                connection_id
            );
            None
        }
