...
use std::{thread};
use crate::networking::messages::{SubscribeTopic, TcpClientMessage, TcpServerMessage, UpdateMode, UserRole};
...
where F : Fn(String) -> Result<String, String> + Send + Sync + 'static + Clone {
    ...
    for stream in listener.incoming() {
        match stream {
            Ok(stream) => {
                println!("\x1b[34mNew client connected: {}\x1b[0m", stream.peer_addr().unwrap());
...
where F : Fn(String) -> Result<String, String> + Send + Sync + 'static + Clone {
    match msg {
        TcpClientMessage::Connect {password, user_name, user_role} => {
            ...
            println!("\x1b[34mDer Nutzer {} hat sich als {} mit Passwort {} verbunden!\x1b[0m",
        ...
        TcpClientMessage::Reconnect {password, user_id, clear_subscriptions} => {
            if clear_subscriptions {
                println!("\x1b[34mDer Nutzer mir der ID {user_id} hat sich mit Passwort {password} neu verbunden und\
        ...
        TcpClientMessage::Subscribe {topic, update_mode} => {
            ...
            println!("\x1b[34m{}\x1b[0m", match update_mode {
        ...
        TcpClientMessage::Unsubscribe { topic } => {
            println!("\x1b[34mDer Client will nichts von {} wissen.\x1b[0m", match topic {
                SubscribeTopic::FixturePositions => "Fixture Positions",
                SubscribeTopic::Universes => "Universes",
            });
            None
