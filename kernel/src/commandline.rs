...
fn new_fixture_type(mut args: SplitAsciiWhitespace) -> Result<String, String> {
    ...
    match fixture_type {
        Err(ChannelError(ChannelAlreadyInUse(channel_type))) => Err(format!(
            "Error: The channel {channel_type} overlaps with another channel."
        )),

        Err(ChannelError(ChannelOutOfRange)) => Err(
            "Error: A Channel is higher than the size of the Universe. This is not yet supported"
                .to_string(),
        ),

        Err(FixtureTypeNameAlreadyInUse(name)) => Err(format!(
            "Error: The Fixture type name {name} is already used."
        )),

        Err(InvalidPropertyType(property_type)) => Err(format!(
            "Error: \"{property_type}\" is not a valid PropertyType"
        )),

        Err(MultipleColorOutputTypes(error_message)) => Err(error_message),
        ...
        Ok(()) => Ok(format!("{} created successfully", name)),
    }
}
...
fn new_fixture(mut args: SplitAsciiWhitespace) -> Result<String, String> {
    ...
    if let Err(_) = channel.parse::<u16>() {
        return Err(format!(
            "Error: \"{fixture_type_name}\" is not a valid channel-number"
        ));
    }
    let channel = channel.parse::<u16>().unwrap();
    ...
    if let Err(_) = universe.parse::<usize>() {
        return Err(format!(
            "Error: \"{fixture_type_name}\" is not a valid universe-number"
        ));
    }
    ...
    match fixture {
        ...
        Err(ChannelError(ChannelAlreadyInUse(overlapping_fixture))) => Err(format!(
            "Error: At least one Channel of this fixture is overlapping with {}. Fixture has not been created.",
            overlapping_fixture
        )),

        Err(InvalidFixtureType(fixture_type_name)) => Err(format!(
            "Error: There is no fixture-type named \"{fixture_type_name}\"."
        )),
...
fn set_value(mut args: SplitAsciiWhitespace) -> Result<String, String> {
    ...
    match result {
        ...
        Err(MissingProperty(_)) => Err(format!(
            "Error: \"{fixture_name}\" has no property \"{property_name}\""
        )),

        Err(InvalidFixture(name)) => Err(format!("Error: \"{name}\" is not a valid Fixture")),
        ...
        Ok(_) => Ok(format!(
            "Value {property_name} of {fixture_name} changed successfully to {value}"
        )),
    }
}
...
fn get_type(mut args: SplitAsciiWhitespace) -> Result<String, String> {
    ...
    match Fixture::get_fixture_type_from_string(fixture_name.clone()) {
        Ok(fixture_type) => Ok(format!(
            "\"{fixture_name}\" is a fixture of the type \"{fixture_type}\""
        )),
